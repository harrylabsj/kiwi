...
function seed(db: DatabaseSync, operationId = "op-1", candidateId = "candidate-1"): void {
  ...
  ).run(MERCHANT, candidateId, `confirmation-${operationId}`, operationId, `sha256:${"a".repeat(64)}`, stamp);
  db.prepare(
...
describe("Workbench persistent reconciliation worker", () => {
  it("expired lease is fenced: the old worker cannot commit after a new token is issued", () => {
    ...
    expect(
      (db.prepare("SELECT status FROM workbench_approval_operations WHERE operation_id='op-1'").get() as {
        status: string;
      }).status,
    ).toBe("succeeded");
  });
  ...
  it("UNKNOWN never re-executes the outbox and later query can resolve the same operation", async () => {
    ...
    const job = db
      .prepare("SELECT next_attempt_at FROM workbench_reconciliation_jobs WHERE operation_id='op-1'")
  ...
  it("five unresolved queries warn; ten escalate to human without resending the action", async () => {
    const { db, store, setNow } = fixture();
    seed(db);
    const execute = vi.fn(async () => ({ status: "unknown", error: "timeout token=secret" }) as const);
    ...
    expect(
      (db.prepare("SELECT status FROM workbench_reconciliation_jobs WHERE operation_id='op-1'").get() as {
        status: string;
      }).status,
    ).toBe("human_required");
    expect(
