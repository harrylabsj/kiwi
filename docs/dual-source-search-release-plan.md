...
| # | 决策 | 选项 | 我的建议 |
| --- | --- | --- | --- |
| 1 | 版本号 | (a) 本次发 **0.11.0**（仓库现值，merchant 批次与本次一并发布）(b) bump 到 0.12.0 让双来源单独可追溯 | **(a)**：0.11.0 从未发布，不构成版本复用；分批发布无额外收益 |
| 2 | 合并方式 | (a) `main` 上 fast-forward（历史线性）(b) PR + review | **(a)**，与仓库现有线性历史一致 |
| 3 | 连接器是否同期 republish | (a) 同期（用户尽早看到 `network_search`）(b) 先只发运行时 + 专家包，连接器下一批 | **(b)**：连接器无仓库源码，republish 是本计划唯一未经本地验证的步骤，应与已验证的步骤分开 |
