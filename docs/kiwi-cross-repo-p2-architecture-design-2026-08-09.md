...
- shopping-cli T9 当前批次完成 `24d7eaf`：将 `merchant_daemon.py` 中无副作用的状态目录、merchant id 安全归一化、agent 路径、回复计数与永久错误判定抽出到 `agents/merchant_state_helpers.py`，保留进程启动/停止、锁、信号、SQLite、文件 I/O 与 facade alias identity；新增 leaf characterization tests。独立验收目标集 102 tests + 4 subtests、全量 pytest 812（194 subtests）、`scripts/verify.sh` unittest 572（9 skipped）+ Node 7 + 额外 6 项、Ruff、mypy 90 source files 全部通过；候选组合锁已同步至 shopping `24d7eaf1e894f84cbb18364e31934d19da5de449`。
...
- 最新候选锁（register input leaf 与组合审计证据后）已更新并再次通过 `verify:portfolio-lock`：kiwi `a3dada19b881f84f6d3ac000a03a855c9a23b5a6`、kiwi-catalog `b2148d52fbbbf27dd893373848eb0e892a8f3bb6`、shopping-cli `b76703ff3d75231556f4e64a21cd1fb44abd9cc9`；三仓 dry-run 均可达，公开锁仍保持 fail-closed。
