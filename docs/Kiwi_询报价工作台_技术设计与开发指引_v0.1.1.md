...
文档日期：2026-09-19。设计状态：开发基线提案。代码核查基线：`harrylabsj/kiwi`，提交 `866dbe69583e3330198696861380043ccc721116`，`package.json` 版本 `0.10.0`。本设计版本与 Kiwi 软件版本分别管理，不能将 v0.1.1 理解为 Kiwi 的降级版本。

v0.1.1 修订（2026-09-19，仅定义补齐，无架构与范围变更）：补齐 CANCELLED／CLOSED／EXPIRED 的进入接口与触发者；为 current_quote_revision 指针与报价生命周期事件指定存储落点；明确 stage 以 current_revision 重算及 READY／PRICED 回退边；限定 EXPORTED 只由可信管理页认证下载触发；对齐 record_delivery 的证据语义；澄清 CSV 契约、工具同步／作业边界、时间戳语义与幂等保留例外；工具清单增至 14 个（新增 kiwi_merchant_rfq_close）。
...
会话辅助写：保存询盘、字段修订、SKU 选择、草稿，以及操作者自述的发送／移交证据记录（只记自述等级）。需要认证与幂等，但不产生对外商业承诺；允许模型提出并记录为未批准工作成果。
...
同版包的 `02_contracts/` 是上述八种对象的候选 JSON Schema。持久化对象 Schema 与工具输入 Schema 分开：模型工具绝不能因为记录中存在 merchant_id，就被允许自行决定租户。契约命名空间与 schema_version 在候选阶段保持 0.1.0，与设计文档版本分别管理；v0.1.1 对契约的改动均为可选字段增量，不破坏既有样例与算例。
...
首版 CSV 输入遵循 RFC 4180：逗号分隔、双引号转义、CRLF／LF 均可；首行必须是 UTF-8 表头（允许带 BOM，服务端剥离），列名必须匹配既定字段映射，未识别列拒绝而非忽略。解析失败、编码非法或列不匹配时整文件拒绝并返回 SOURCE_INVALID，不截断后继续正式报价。
...
所有服务端时间戳（received_at、verified_at、valid_until、expires_at 等）一律以 UTC ISO 8601 存储与比较，展示层再本地化；新鲜度与超期判定一律以服务端时钟为准。
...
首版仅 CNY，金额单位为分。JSON 金额使用 0 至 1,000,000,000,000 的安全整数；单个报价总额也不得超过该上限。数量为 1 至 100,000 的整数，每份询盘最多 100 行。内部乘法与税额用 BigInt 或等价精确整数运算，禁止用二进制浮点先算元再乘 100。行级中间量（quantity × unit_price_minor、行优惠后金额、单行税额）与运费、总额受同样上限约束；任一中间量越界即按 PRICING_INVALID 整单拒绝，不得截断、取模或回退到浮点。
...
| 状态域 | 状态 | 进入条件／退出动作 |
| --- | --- | --- |
| RFQ | NEW → NEEDS_CLARIFICATION → READY → PRICED → CLOSED | 需求、SKU、计价字段齐备才 READY；报价成功才 PRICED；确认撤回或事实冲突回退 NEEDS_CLARIFICATION |
| RFQ | CANCELLED／CLOSED | 操作者通过 kiwi_merchant_rfq_close 或管理页同义路由显式取消／关闭；保留既有审计与已导出版本 |
| 报价 | DRAFT → VALIDATED → PENDING_APPROVAL | 通过确定性校验，并登记冻结内容的候选 |
| 报价 | APPROVED → EXPORTED | 可信批准使产物可下载；仅可信管理页认证主体的成功产物下载计 EXPORTED |
| 报价 | REJECTED／SUPERSEDED／EXPIRED | 拒绝、前置变化／新版本替代、超期（EXPIRED 由 Core 按 valid_until 在读取与前置校验时惰性判定）；不可再次放行 |
...
内部 quote 内容不可变，生命周期变化通过事件／投影表维护；当前状态以最新事件投影为准，quote 记录的 status 字段随之推进。`rfq_revision` 与 `quote_revision` 不可共用同一个计数器。一份 RFQ 可有多版报价，但同时只允许一个 current_quote_revision；该指针保存在 rfq_cases 上（见 10.1），随版本替代、失效与 case 关闭同步更新。

stage 是 case 级投影，判定对象始终是 current_revision 指向的需求 revision：创建新 revision 后按新 revision 的必填字段确认情况重算（未确认 → NEEDS_CLARIFICATION，齐备 → READY），不继承旧 revision 的 READY／PRICED。允许的回退边：READY／PRICED → NEEDS_CLARIFICATION（新 revision 关键字段未确认、事实源冲突或操作者撤回确认）。PRICED 仅当 current_quote_revision 指向有效（非 REJECTED／SUPERSEDED／EXPIRED）报价时保持；指针失效即回退 READY。

CANCELLED／CLOSED 只能由操作者进入：通过 kiwi_merchant_rfq_close 工具（prepare 级、幂等、要求 expected_revision 与 outcome）或管理页同义路由。进入时未决审批候选一并失效，后续确认返回 APPROVAL_STALE；终态 case 拒绝新的计价、发布与确认请求（CASE_CLOSED），已导出文件与审计保留。

## 8.2 转移前置条件

READY 要求所有必填字段已确认、没有关键冲突。VALIDATED 要求价格、单位、税、运费和交期有依据且可重算。PENDING_APPROVAL 要求正式产物已渲染、摘要固定、审批候选落盘。APPROVED 要求可信主体批准、一次性凭证有效、事实与策略重验通过、当前版本一致。EXPORTED 要求可信管理页认证主体成功取走已批准的固定产物（下载记录含文件摘要校验），而非仅生成下载入口；MCP 产物资源通道的读取只记录访问事件，不推进该状态。
...
| 表 | 主要字段／约束 |
| --- | --- |
| rfq_cases | case_id PK；merchant_id；current_revision；current_quote_revision（可空）；stage；version |
...
| rfq_quote_events | quote_id + revision；事件（VALIDATED／PENDING_APPROVAL／APPROVED／EXPORTED／REJECTED／SUPERSEDED／EXPIRED）；actor；时点；原因；最新事件为当前状态投影 |
...
建议幂等记录保留 30 天；prepare_release 与 release 激活类操作的幂等记录跟随对应报价保留策略清理，不得提前。保留时长需在试点中与商家确认；不能把清理后的旧键承诺为永久幂等。
...
interface RfqService {
  ...
  close(ctx: AuthContext, cmd: CloseCommand): Promise<RfqCase>;
...
| MCP tool（建议名称） | 关键输入 | 同步／作业 | 权限与结果 |
| --- | --- | --- | --- |
| kiwi_merchant_rfq_ingest | source、client_ref、idempotency_key | 大材料可作业 | workflow prepare；保存未批准询盘 |
| kiwi_merchant_rfq_get | case_id | 同步 | read；当前需求及阻断项 |
| kiwi_merchant_rfq_revise | case_id、expected_revision、changes、key | 同步 | prepare；产生新 revision |
| kiwi_merchant_rfq_close | case_id、expected_revision、outcome（CANCELLED／CLOSED）、reason、key | 同步 | prepare；操作者显式取消或关闭 case；未决候选一并失效 |
| kiwi_merchant_rfq_match | case_id、line_id、query、cursor | 同步 | read；商品候选，不自动确认 |
| kiwi_merchant_rfq_confirm_items | case_id、expected_revision、selections（每项含 confirmation_ref）、key | 同步 | prepare；消费服务端已验证的确认引用，不接受模型自报确认 |
| kiwi_merchant_rfq_refresh_facts | case_id、expected_revision、key | 同步 | prepare；建立新事实快照 |
| kiwi_merchant_rfq_price | case_id、expected_revision、snapshot_id、key | 同步 | prepare；Core 计算，不收模型最终金额 |
| kiwi_merchant_rfq_compare | from_quote_ref、to_quote_ref | 同步 | read；字段级差异与失效原因 |
| kiwi_merchant_rfq_prepare_release | quote_ref、recipient_ref、expected_revision、key | 渲染可作业 | prepare；只生成批准候选与预览 |
| kiwi_merchant_rfq_get_release | release_id | 同步 | read；批准／导出状态和授权产物引用 |
| kiwi_merchant_rfq_record_delivery | quote_ref、channel、evidence_ref、key | 同步 | prepare；只记操作者自述 REPORTED_SENT，必须引用操作者提供的证据，证据等级不可由模型提升 |
| kiwi_merchant_rfq_prepare_handoff | quote_ref、target_ref、intent_evidence_ref、key | 打包可作业 | prepare；只准备移交包的批准 |
| kiwi_merchant_rfq_get_job | job_id | 同步 | read；作业状态 |

标为“可作业”的工具在输入解析或渲染可能超过单次交互预算时返回 job_id，其余工具同步完成；作业边界与恢复语义见 10.4。
...
| SOURCE_INVALID | 来源材料无法解析或超出导入契约 | 否，修正材料后重新导入 |
...
| CASE_CLOSED | 询盘已取消或关闭 | 否，不创建新工作成果 |
...
建议以实例私有路径 `/admin/rfq/` 承载浏览与确认；写接口包括 `POST /admin/rfq/releases/{id}/approve`、`POST /admin/rfq/releases/{id}/reject` 和 `POST /admin/rfq/cases/{case_id}/close`（取消或关闭询盘，与 kiwi_merchant_rfq_close 工具共用同一命令语义）。同源会话、CSRF 和一次性确认凭证全部有效才受理；case close 按管理页会话主体授权，不需要一次性确认凭证。服务端从会话解析 actor，不接受任意 merchant_id／principal_id。

下载接口可为 `GET /admin/rfq/artifacts/{artifact_id}`，始终认证并校验归属，不把带长期访问凭据的下载链接放进模型；认证主体的成功下载（含摘要校验）是 EXPORTED 的唯一触发通道。MCP 产物资源读取走其已认证通道，但只记录访问事件、不推进状态；具体宿主文件保存行为列入实机测试。
...
正式文件建议 PDF + 纯文本摘要，数据来自同一 PublicQuoteView。PDF 字体嵌入及许可证由项目负责；不得要求用户安装陌生字体。渲染路线（无头浏览器或 PDF 库）在 RFQ-010 中比较后固定，标准：中文字体嵌入、无网络资源依赖、同输入可重复渲染；选型结果与模板版本一同登记。HTML 只作为内部渲染媒介，不允许任意网络资源、脚本或模板表达式。
...
详见 `05_acceptance/acceptance-matrix.json` 的80项 Given／When／Then 规格及状态。测试 ID、阶段和证据路径固定，方便开发代理逐项回填；初始不能预填“通过”。80 项覆盖 M1–M4 的产品与技术验收；L3 试点与 M6 扩展按第 20 章指标另行规格化，不计入本矩阵。
...
| WorkBuddy 云端到实例 MCP 的可达路径 | 私有试点优先受控网络直连 | staging 实测的暴露面批准、连通性与每商家 OAuth 配置证据 |
...
`kiwi-rfq-workbench-v0.1.1/` 中包含本设计 Markdown、八份候选 JSON Schema、样例、参考计价实现与算例、80项验收规格、三个技能草稿、MCP 工具设计契约、来源锁定记录、文件清单与自检报告。所有文件实际打包，不引用一个不存在的“另附交接包”。
