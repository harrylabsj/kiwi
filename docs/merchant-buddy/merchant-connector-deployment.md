...
> **版本口径（2026-09-19，询报价设计 v0.1.1 §2.3 漂移修正）**：0.10.0 起的当前
> 边界以本节为准——网关只做目录身份与公开资料，不持有商家实例地址/凭据，
> 不代理实例工具；实例的 `/mcp`、`/oauth/*` 不对公网。本文后文出现的
> 「网关走 loopback 调用实例」等表述是历史迁移语境（同机过渡拓扑），
> 与"不碰实例"冲突时以本节与 CHANGELOG 0.10.0 为准，不得据此恢复旧路由。

**网关的职责边界 = 第 0 版目录能力。** Buddy 引用网关做**商家注册与发布公开资料**
（连接商家身份、起草草稿、请求发布、查询状态、撤回）。

硬边界（不因功能需求而放宽）：

- **不持有**商家实例的地址、端口或任何调用凭据；
- **不代理**实例工具（不提供 `kiwi_merchant_*` 一类转发）；
- **不参与**买家与商家之间的 A2A 磋商。

**商家实例完全独立**：它跑在商家自己的服务器上 7×24，提供 A2A 接待（agent card +
磋商），向目录注册自己的公开身份，**不与网关建立任何连接**。买家（含采购询价专家）
经目录发现商家后，**直接**与商家实例做 A2A——整条链路不经过网关。

这样切分的理由：

1. **信任面**：网关若持有各商家的实例调用凭据，一旦被攻破就是**全部已绑定实例**的
   调用能力；不持有则无从波及。
2. **责任面**：网关不做商家自托管服务的必经中间人，就不会把商家的可用性、延迟与
   故障揽到自己身上，故障域天然分离。
3. **与最小授权一致**：不签发凭据、不做吊销列表、不碰商家私有状态——"不碰实例"
   是这条线的终点。

### 保留但**当前不采用**：网关侧实例路由

仓库里已实现一套网关↔实例的绑定与路由（`/instance` 绑定页、配对码
`kiwi merchant mcp pair`、`tenants.json` 静态租户、`kiwi_merchant_*` 工具代理）。按本
原则**当前不启用**，代码保留作兜底——**仅当**将来确认平台的连接器只能配固定 MCP
地址、而商家又无法自行暴露实例时，才重新评估启用（届时需重过
[配对设计](merchant-instance-pairing-design.md) 的威胁模型）。

启用会与本文档 §4（实例绑定表单与 env）、§5（实例侧配置）、§6（目录侧 connector env）
以及 §9 的绑定相关验收相关；**未启用时这些章节均不适用**，按空 `tenants.json` 运行即可。

### 注意：不要把"未绑定实例"当成故障

第 0 版能力**不依赖**实例：商家未部署、未运行、不可达，都不影响目录注册与发布。
`kiwi_merchant_*` 工具**本就不该出现**，不要为此排障。
