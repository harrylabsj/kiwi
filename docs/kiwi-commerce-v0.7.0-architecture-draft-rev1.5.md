...
只有经过 NetworkDisclosurePolicy、HardPolicy 和 Approval 明确允许的最小字段才可以对外披露。
...
任何 Condition 或 RFQ 想使用上述属性，都必须先通过 NetworkDisclosurePolicy。
...
# 33. Commerce Discovery / Channel / Data 职责

## 33.1 Buyer Discovery Starts from Need

默认 Happy Path 不是先知道 Merchant，而是：
...
因此 `AgentDiscovery` 与 `CommerceDiscovery` 分层：

```text
CommerceDiscovery
  ├── KiwiCatalogListingSource
  └── future commerce sources
          ↓
CommerceCandidate[]
          ↓
AgentDiscovery
...
`CommerceDiscovery` 回答“谁可能满足需求”；`AgentDiscovery` 回答“这个 Agent 当前是谁、在哪里、怎么连接”。
...
ProductListing indexing/search
CapabilityListing indexing/search
Merchant public projection
profile/listing freshness
trust metadata
...
```text
product master data authority
real-time inventory authority
price authority
merchant floor/cost
...
```text
Listing = discovery projection
not authoritative commerce state
```

## 33.3 CounterpartyChannel
...
发现 Merchant Agent 并完成 fresh verification 后，Direct A2A 正常路径绕过 kiwi-catalog。

## 33.4 CommerceDataSource
...
delivery
after-sales
public listing inputs
authorized business context
```

实现 MAY 包括 ShoppingCliDataSource / ERP / PIM / local DB / platform API。

## 33.5 Two-stage Product Discovery

```text
Stage 1 — Cheap Discovery
kiwi-catalog Listing
= fast / searchable / public / possibly stale hint

Stage 2 — Authoritative Inquiry
Merchant Kiwi + CommerceDataSource
= current / authenticated / negotiable / auditable
```
...
Listing 只是 public search projection。
...
```text
shopping-cli = 商家当前真正有什么，以及这些事实来自哪里
```

增加 `PublicListingProjection`，把 Merchant 授权公开的商品/能力摘要推送到 kiwi-catalog。

## 34.3 Relationship

```text
kiwi-catalog
= commerce discovery projection

A2A
= direct agent communication

shopping-cli
= merchant commerce source/data hub

KNP
= authoritative negotiation semantics

KTH
= Agreement → real transaction destination
```
...
→ Listing Search
→ Merchant Agent
→ fresh Agent/Card verification
→ Direct A2A
→ shopping-cli / ERP current facts
→ KNP Offer/Negotiation
→ KTH
```
...
在 Direct A2A 开发前先完成现有 Hosted Runtime 问题（决策依据：hosted negotiation 是当前唯一在线协商通道，见 shopping-cli v0.3 §10 存续承诺；P0 修复是维持存量可用性的必要投入，不是给即将退役的系统追加投入）：
...
docs/kiwi-a2a-v1.0-readiness-audit-2026-08-06.md
...
22. kiwi-catalog 支持 ProductListing 与 CapabilityListing。
23. Listing 搜索结果绑定 `owner_agent_id`，但不复制 endpoint 作为 Listing authority。
...
docs/products/
  ...
  shopping-cli-commerce-data-hub-v0.3.md
...
# Appendix — rev1.5 Product-first Discovery Upgrade

本修订把默认 Buyer Discovery 从 Merchant-first 调整为 Product/Capability-first，同时保持 Direct A2A、Agent verification 和 Commerce Source of Truth 边界不变。
