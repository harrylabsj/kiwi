...
5. 三个仓库公开后，组合/发布 workflow 直接 checkout 两个公开 sibling 仓，不需要静态 GitHub App token、PAT 或 registry secret。
   - 不要把个人宽权限 CLI token、registry token 或私钥写入 workflow；发布身份只使用受保护环境中的短期 OIDC。
   - `portfolio-integration.yml` 保持 dispatch-only，普通 PR 由各仓 CI 覆盖，组合门禁在受控 dispatch 上执行。
