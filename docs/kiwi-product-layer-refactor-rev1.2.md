...
| D2 | `kiwi merchant publish` 编排 | 一次命令完成：agent 注册（kiwi-catalog，owner token 派生）→ listing 发布（进程调用 shopping-cli `listings projections list` 只读投影 → 逐条直连 kiwi-catalog `/v1/listings/publish`，服务端行级幂等；reconcile 对投影消失商品 withdraw）→ 分步状态汇总（fail-closed：任一步失败非零退出 + 明确原因）；编排 E2E（双 stub）通过 | ✅ 已实现（kiwi e1fe866 + 2026-08-08 v3.0 适配）：三步编排 + 幂等（先查复用）+ 身份统一 + 版本门（D3 矩阵消费）；发布面随 shopping-cli v3.0 移交 kiwi-catalog |
| D3 | `kiwi doctor` 聚合健康 | 输出三组件状态：Kiwi runtime self-check + shopping-cli 存在性/版本（版本兼容矩阵 `Kiwi x.y supports shopping-cli >= a.b`，v2.0 起含 projections 只读命令、v3.0 发布面移交 kiwi-catalog）+ kiwi-catalog 可达性；矩阵有单一来源配置并被 doctor 与 publish 共同消费 | ✅ 已实现（kiwi e1fe866）：product-compat.ts 单一来源（>= 2.0.0 不限上限，v2/v3 均兼容），doctor 与 publish 共同消费 |
| D4 | `kiwi buyer` 命令面 | `buyer init / start / search / tasks` 可用：search 复用 Product-first 链路（listing 搜索，经 KiwiCatalogSource.searchListings；无独立 ProductIntent 类型，参数工具平铺），start 为 chat TUI 入口；Buyer 不感知 shopping-cli / kiwi-catalog（§2.2） | ✅ 已实现（kiwi e1fe866 + kiwi-catalog 07bad01）：init/search/tasks + start 别名；search 复用 M3 链路；跨仓修复 merchants 影子表写入方 |
