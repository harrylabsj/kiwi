...
date: 2026-08-07
target_implementation: Kiwi v0.6.0
scope: Pre-transaction Agent-to-Agent commerce negotiation
---

# Kiwi Negotiation Protocol 1.0

**Kiwi A2A v0.6.0 已发布（2026-08-07）**（产品版本由 v1.0.0 回退，KNP 协议身份不变）：基线 §41 完成定义 27/27 经就绪度审计实证满足。
...
Nesting depth MUST NOT exceed 2 below the root condition node.
...
1. clone the envelope;
2. remove `digest`;
3. remove any transport-specific signature fields not defined by KNP;
4. canonicalize with RFC 8785 JCS;
5. compute SHA-256 over UTF-8 canonical bytes;
6. encode as lowercase hexadecimal prefixed by `sha256:`.
...
KNP/1.0 becomes the canonical new negotiation domain.
...
# 36. Version 1.0 Completion Criteria

KNP/1.0 is implementation-ready only when:

1. all schemas in `schemas/` validate;
2. normative examples validate against their schemas;
3. digest vectors are reproducible;
4. condition vectors are deterministic;
5. transition vectors pass;
6. duplicate message behavior passes;
7. A2A binding is interoperable with an independent implementation;
8. UCP capability advertisement passes namespace-origin validation;
9. legacy adapter loss cases fail closed;
10. no-order/no-payment/no-reservation invariants hold.
