...
| D2 | `kiwi merchant publish` 编排 | `src/product-publish.ts`：三步编排——(0) shopping-cli 版本门（D3 矩阵消费，fail-closed）、(1) agent 确认/注册（**幂等：先按 merchant 查询复用，没有再注册**；owner token 与 kiwi-catalog 逐字节一致；一商家一 agent 409 由此规避）、(2) 进程调用 shopping-cli **`listings projections list`** 只读投影（--db 顶层参数顺序、60s 超时、spawn ENOENT 细节）→ 逐条直连 kiwi-catalog `POST /v1/listings/publish`（owner token 双路径、白名单 wire 字段、空 category 跳过）→ reconcile 对投影消失商品 `POST /v1/listings/{id}/withdraw`（DoD #5）；分步汇总 fail-closed（2026-08-08 修订：shopping-cli v3.0 剥离 publish-listings 后发布面归 kiwi-catalog） | `tests/product-publish.test.ts`（10 例：成功路径/参数构造/owner token 固定向量/幂等复用/agent 短路/不兼容版本短路/_provenance 剥离与空 category 跳过/withdraw diff/自查失败 fail-closed） | ✅ |
| D3 | `kiwi doctor` 聚合健康 | `src/product-compat.ts` 版本兼容矩阵**单一来源**（SHOPPING_CLI_COMPAT = >= 2.0.0 不限上限 + parseVersion/compareVersions/versionInRange，parse 失败 fail-closed；projections 只读命令 v2.0 起提供，v3.0 发布面移交 kiwi-catalog——v2/v3 均兼容）；doctor 输出三组件状态（shopping_cli 含 compatible 判定与"超出支持范围"明细） | `tests/product-compat.test.ts`（7 例：解析/比较/范围/上下边界/fail-closed）+ product-cli doctor 结构断言 | ✅ |
