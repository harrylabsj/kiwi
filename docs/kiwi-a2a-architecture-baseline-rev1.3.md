...
supersedes: "doc_revision 1.2"
scope: Agent-to-Agent Commerce through Safe Transaction Handoff
---
...
| shopping-cli catalog | CommerceDataStore | 商品、库存、价格、配送和经营数据，不再承担 Agent Catalog |
...
# 34. kiwi-catalog 与 shopping-cli 的产品定位

## 34.1 kiwi-catalog

`kiwi-catalog` 从 shopping-cli 中独立出来，成为单独产品：

> **Commerce Agent Catalog + Discovery & Verification Infrastructure**

它回答：

> 有哪些商业 Agent？它们代表谁、会什么、在哪里、如何连接、最近是否验证过？
...
Agent registration
Agent search
Agent Card cache/index
UCP Profile cache/index
capability index
verification
freshness
trust metadata
hosted/direct metadata
```

它不拥有 Merchant 商品库存的最终 authority，也不进入 Direct A2A 的消息路径。
...
shopping-cli 不再承担 Agent Catalog。

新的主定位：

> **Merchant Commerce Data & Operations Hub**

它可以成为 Merchant Kiwi 的本地或服务端商品数据库，并通过开放接口接入：

```text
ERP
PIM
local SQLite/Postgres/MySQL
CSV / Excel import
merchant internal API
inventory system
pricing system
delivery system
existing marketplace/platform API
```

shopping-cli 对 Merchant Kiwi 提供统一的：
...
现有 `shopping.negotiation/0.1`、claim/heartbeat/audit/policy gate MAY 继续作为 hosted/legacy negotiation infrastructure 保留，但它不再承担 network-wide discovery。
...
```text
shopping-cli ← ERP / local DB
      │
      ▼
Merchant Kiwi
      │
      ├── publish public Agent metadata → kiwi-catalog
      └── negotiate directly → Buyer Agent
```

商品私有数据不得因为注册到 kiwi-catalog 而自动公开。
...
> **让 Kiwi 在不承担订单/支付/履约责任的前提下，把 Agent 已经谈妥的方案带到真实成交入口。**
...
`destination_payload` 必须是最小化、schema-validated、非秘密数据。

## 35A.3 Destination Types
...
外部交易结果必须由外部 authority 明确回传，且未来应由独立 Transaction/Order integration 规范管理。

## 35A.5 Selection Rules

优先级建议：

```text
1. standard UCP checkout/order capability
2. merchant-controlled checkout URL/API
3. buyer enterprise ERP/procurement workflow
4. platform deep link
5. PO / quote document
6. merchant contact handoff
```

Kiwi SHOULD 选择最可验证、最少权限、最可审计的 destination。

## 35A.6 Agreement Binding

执行 Handoff 前 MUST 重验：

```text
agreement still valid
terms_digest unchanged
destination still valid
counterparty identity unchanged
...
expose private budget
expose merchant cost/floor
auto-submit payment
auto-confirm an order unless a future explicit transaction protocol authorizes it
```
...
```text
HTTPS policy
allowlist / trust policy where appropriate
redirect controls
phishing checks
display destination to user
```

## 35A.8 Commercial Metrics

第一阶段不要只看“原生订单数”，而应看：

```text
request_to_discovery
discovery_to_rfq
rfq_to_offer
offer_to_agreement
...
# 41A. Kiwi Commerce v1.1 Completion Definition
...
- KNP/1.0 继续是 pre-transaction negotiation 的 normative wire spec；
- `kiwi-transaction-handoff-0.1.md` 定义 Agreement 之后的安全交接；
- kiwi-catalog 与 shopping-cli 是独立产品，不由 KNP 规范控制其内部实现；
